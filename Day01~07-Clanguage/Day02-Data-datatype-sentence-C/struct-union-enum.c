...
char 强制对齐要求为 min(1, 2) = 1（要求偏移量是 1 的倍数，也就是任意位置都可以放）。

short 强制对齐要求为 min(2, 2) = 2（要求偏移量是 2 的倍数）。

int 强制对齐要求为 min(4, 2) = 2(原本要求 4 的倍数，被 pack(2) 降级成了 2 的倍数）。

所以，在 pack(2) 下,int i 可以紧跟在 char c2 后面，不需要为了凑 4 字节而对齐。
