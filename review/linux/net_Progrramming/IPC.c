...
/*
数据传输和读写的主要问题：
// 会用 epoll（LT/ET都懂，知道ET为什么要循环读）。
// 会解决 TCP 粘包/半包（状态机、定长、长度字段）。
// 会处理 断线重连、心跳包。
// 懂 多线程/线程池，别让耗时的业务把网络IO卡死。
...
//拓展：Reactor 模式，其实就是“用一个管家（epoll）统一监听所有客人，有事了再叫对应的服务员”。它是 Linux 高并发服务器（比如 Nginx、Redis、Netty）的核心架构。
//核心思想：事件驱动 + 回调函数。Reactor 模式本质上就是“epoll + 回调函数”的封装
/*
// 伪代码：极简 Reactor 核心逻辑
while (1) {
    // 1. 大堂经理巡视：有没有人举手？
    ...
    for (int i = 0; i < n; i++) {
        ...
        // 2. 根据事件类型，分发给不同的处理函数（回调）
        if (events[i].events & EPOLLIN) {
            if (fd == listen_fd) {
                // 迎宾：新客人来了，交给 accept 处理
                acceptor_callback();
            } else {
                // 服务员：老客人发消息了，交给 read 处理
                read_callback(fd);
            }
        } else if (events[i].events & EPOLLOUT) {
            // 客人准备接收消息了，交给 write 处理
            write_callback(fd);
        }
    }
}
...
1.单 Reactor 单线程（好比只有一个全能员工）
代表：Redis。
做法：一个 epoll 循环，既负责 accept，又负责 read，还负责执行所有的业务代码。
缺点：一旦某个业务卡住，整个服务器就卡死。不能发挥多核 CPU 性能。

2. 单 Reactor 多线程（大堂经理 + 一群服务员）
做法：Reactor 线程只管 accept 和 read（收数据）。收完数据后，丢给线程池去处理复杂的业务逻辑。处理完再丢回给 Reactor 发送。
优点：业务处理不阻塞网络 IO。
缺点：单个 Reactor 如果扛不住几十万连接，就会成为瓶颈。

3. 主从 Reactor 多线程（行业标准，Nginx/Netty 都在用）
做法：
主 Reactor：只负责 accept 新连接。
拿到新连接后，把 fd 分发给多个子 Reactor。
每个子 Reactor 在自己的 epoll 里监听读写事件，然后交给线程池处理业务。
优点：完美榨干多核 CPU 性能，能扛几十万并发。


如果面试官问：“之前的 MCU 网关或手表中间件，有接触过高并发服务器吗？”
答：
“我之前的项目以 MCU 的 FreeRTOS 和手表中控为主。在自学 Linux 系统编程时，我重点研究了网络并发模型。
我写过一个基于 epoll ET 模式的 TCP 服务器 Demo，采用了单 Reactor 多线程的架构。
用一个 epoll 实例监听监听套接字和客户端套接字，收到数据后交给线程池处理，避免耗时业务阻塞网络 IO。
在面试前，我正在进一步学习主从 Reactor 架构，比如 Nginx 的实现思路。”
有高并发架构的思维，不是只停留在“会写个 socket”。
